...
const Dashboard: React.FC = () => {
  ...
  const filteredUserChatbots =
    (userChatbots as Chatbot[])?.filter((chatbot: Chatbot) => {
      const matchesSearch =
        chatbot.name?.toLowerCase().includes(searchQuery.toLowerCase()) ||
        false ||
        chatbot.description?.toLowerCase().includes(searchQuery.toLowerCase()) ||
      ...
      <div className='p-4'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-4 gap-6 mb-8'>
          <div className='bg-card rounded-lg shadow-sm border border-border p-6'>
            ...
            <div className='text-2xl font-bold text-foreground'>{defaultFilteredChatbots.length + filteredUserChatbots.length}</div>
          </div>
          ...
          <div className='bg-card rounded-lg shadow-sm border border-border p-6'>
            ...
            <div className='text-2xl font-bold text-foreground'>
              {defaultFilteredChatbots.filter((c: Chatbot) => c.status === 'active').length + filteredUserChatbots.filter((c: Chatbot) => c.status === 'active').length}
            </div>
          </div>
        ...
        {state?.context?.chatbots && state.context.chatbots.length > 0 && (
          <>
            <h3 className='my-6 text-xl font-semibold text-foreground'>My Chatbots</h3>
            <Chatbots chatbots={[]} useMachineState={true} searchQuery={searchQuery} statusFilter={statusFilter} />
          </>
        )}
      </div>
    </AdminLayout>
