...
import { useSocket, type SocketMessage, type SocketEvents } from './useSocket';
import type { ChatMessage, TypingState, ConnectionState } from '../types/types';

export const useChat = (widgetKey: string, chatbotId: string, socketUrl?: string) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (Math.random() > 0.7) { // 30% chance to request analysis
        setTimeout(() => {
